...
- IPv4 (32 bits) и IPv6 (128 bits) — логические сетевые адреса для маршрутизации между сетями.
- MAC (48 bits) -адрес — канальный адрес сетевого интерфейса для локальной передачи кадров.
...
> Ветка нужна для изоляции работы до готовности. это помогает работать в команде.
...
Практическое задание: Диагностика сетевого подключения компьютера. Определите IP-адреса компьютера, на котором вы работаете, и используемые сетевые интерфейсы. Определите, является ли интернет-адрес частным или публичным, тип подключения "прямое или NAT" и используемую маску подсети. Кратко запишите результаты в отчет.

## Задание 2

Практическое задание: TCP echo server/client на Python. Напишите TCP-сервер, который прослушивает порт 9999, принимает подключение клиента, получает строку с именем студента и отправляет обратно ответное сообщение. Напишите TCP-клиент, который подключается к серверу, отправляет имя студента и выводит ответ сервера. Проверьте работу на localhost. В отчете укажите, какие роли выполняют сервер, клиент, IP-адрес и порт.

## Задание 3

Практическое задание: UDP echo server/client на Python. Напишите UDP-сервер, который прослушивает порт 9998, принимает сообщение от клиента и отправляет ответ обратно на адрес отправителя. Напишите UDP-клиент, который отправляет имя студента серверу и выводит полученный ответ. Проверьте работу на localhost. В отчете кратко сравните UDP-версию с TCP-версией и объясните, почему UDP не устанавливает соединение заранее.

## Задание 4

Практическое задание: многопоточный TCP echo-сервер. Напишите TCP echo-сервер, который способен обслуживать несколько клиентов одновременно с использованием потоков. Каждый новый клиент должен обрабатываться отдельно, чтобы один клиент не блокировал работу других. Напишите простой клиент для проверки, запустите несколько клиентов одновременно и покажите, что сервер отвечает каждому. В отчете объясните, какие проблемы могут возникнуть при работе потоков с общими данными.

## Задание 5

Практическое задание: TCP echo-сервер с использованием нескольких процессов. Напишите TCP echo-сервер, в котором обработка клиентов выполняется с использованием отдельных процессов. Проверьте подключение нескольких клиентов одновременно и покажите, что каждый клиент получает ответ. В отчете сравните этот подход с многопоточным вариантом по изоляции, расходу ресурсов и устойчивости при ошибке в обработчике клиента.

## Задание 6

Практическое задание: Flask-приложение и Docker. Создайте простое Flask-приложение с главной страницей, которая возвращает понятный текстовый или HTML-ответ. Подготовьте файл requirements.txt с зависимостями проекта и Dockerfile для запуска приложения в контейнере. Настройте приложение на прослушивание порта 8000. Соберите образ, запустите контейнер и проверьте доступность страницы через браузер или HTTP-запрос. В отчете укажите структуру проекта и результат проверки.

## Задание 7

Практическое задание: SSH-доступ и передача файлов. На Linux-системе установите и подготовьте SSH-сервер, убедитесь, что служба запущена, настройте файрвол так, чтобы был разрешен доступ к порту 22, затем выполните подключение с другого компьютера или через localhost. Создайте тестовый файл, передайте его на сервер и скачайте обратно с использованием защищенного копирования. В отчете укажите, какие проверки подтвердили работоспособность SSH и передачи файлов.

## Задание 8

Практическое задание: Python-окружение, зависимости и Git. Создайте отдельный каталог учебного проекта, настройте виртуальное окружение Python, активируйте его, установите библиотеку numpy, проверьте список установленных пакетов и создайте файл requirements.txt. Инициализируйте Git-репозиторий, добавьте файл .gitignore для служебных файлов Python и виртуального окружения, выполните первый коммит. В отчете объясните, зачем нужны виртуальное окружение, requirements.txt и .gitignore.

## Задание 9

Практическое задание: простой сайт из нескольких HTML-страниц. Создайте небольшой статический сайт, состоящий из главной страницы index.html и нескольких связанных между собой страниц. Добавьте навигационные ссылки между страницами и краткое содержательное наполнение. Проверьте открытие сайта в браузере. В отчете опишите структуру файлов и объясните, чем статический сайт отличается от динамического веб-приложения.

## Задание 10

Практическое задание: базовая работа с файлами и правами в Linux. Создайте новый каталог учебной работы, перейдите в него и создайте текстовый файл. Настройте права доступа так, чтобы владелец, группа и остальные пользователи могли читать, записывать и выполнять этот файл. Откройте файл в текстовом редакторе, запишите в него свое имя и сохраните изменения. В отчете укажите, какие действия были выполнены и почему права доступа нужно назначать осознанно.

## Задание 11

Практическое задание: простой сканер открытых портов на Python. С использованием сокетов напишите программу, которая проверяет заданный диапазон портов на вашем компьютере или учебном хосте и показывает, какие порты доступны для подключения. Ограничьте диапазон проверки небольшим числом портов, чтобы задание оставалось безопасным и учебным. В отчете объясните, что означает открытый порт и почему сканирование чужих систем без разрешения недопустимо.

## Задание 12

Практическое задание: диагностика сетевого подключения компьютера. Определите IP-адреса компьютера, на котором вы работаете, и укажите используемые сетевые интерфейсы. Найдите основной интерфейс, через который компьютер выходит в сеть. Определите, является ли адрес для доступа в Интернет частным или публичным, и сделайте вывод о типе подключения: прямое подключение или подключение через NAT. Определите маску подсети и адрес сети. Проверьте доступность localhost, собственного IP-адреса, шлюза по умолчанию и любого доступного сайта. В отчете кратко запишите полученные значения и вывод о работоспособности сети.
